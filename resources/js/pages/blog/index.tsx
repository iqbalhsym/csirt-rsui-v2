...
const Pagination = ({ links, isDarkMode }: { links: PaginationLink[]; isDarkMode: boolean }) => {
    ...
    return (
        <nav className="flex flex-wrap justify-center gap-2 mt-12" aria-label="Navigasi halaman blog">
...
export default function BlogIndex({ posts, yearSections, archives, activeYear }: BlogIndexProps) {
    ...
    return (
        <div className={`min-h-screen font-sans transition-colors duration-300 relative overflow-x-hidden ${isDarkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
            <Head title="Berita & Peringatan Siber | CSIRT RSUI">
            ...
            <header className={`sticky top-0 z-40 w-full backdrop-blur-md transition-all duration-300 border-b ${isDarkMode ? 'bg-slate-950/85 border-slate-900' : 'bg-white/90 border-slate-200'}`}>
                <div className="max-w-full mx-auto px-6 md:px-12 lg:px-16 py-2.5 flex items-center justify-between">
                    ...
                    <nav className="hidden lg:flex items-center space-x-8 text-sm font-medium z-10">
                        ...
                        <Link href="/#tentang" className={`transition-colors hover:text-blue-500 ${isDarkMode ? 'text-slate-400' : 'text-slate-650'}`}>Tentang</Link>
            ...
            <div id="top" className={`relative py-20 text-center border-b ${
                ...
                <div className="max-w-4xl mx-auto px-6 relative z-10 space-y-4">
                    <p className={`uppercase tracking-[0.25em] text-xs font-bold ${isDarkMode ? 'text-blue-400' : 'text-blue-200'}`}>Berita &amp; Artikel</p>
                    <h1 className="text-3xl md:text-5xl font-extrabold tracking-tight">
                        Informasi & Peringatan Keamanan Siber
                    </h1>
                    <p className={`text-sm md:text-base max-w-2xl mx-auto ${isDarkMode ? 'text-slate-350' : 'text-blue-100'}`}>
                        Ikuti pembaruan terkini, analisis ancaman, dan panduan mitigasi keamanan digital dari Tim CSIRT RSUI.
                    </p>
                </div>
            </div>
            ...
            <main className="max-w-full mx-auto px-6 md:px-12 lg:px-16 py-16 relative z-10 w-full">
                <div className="grid lg:grid-cols-12 gap-12">
                    ...
                    <section className="lg:col-span-8 space-y-12">
                                ...
                                <div key={section.year} id={getYearAnchor(section.year)} className="space-y-6">
                                    <div className="flex items-center justify-between border-b pb-3 border-slate-900/40">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h2 className="text-xl font-bold">Arsip Tahun {section.year}</h2>
                                        </div>
                                        <span className="text-xs text-slate-500 font-mono">{section.posts.length} rilis berita</span>
                                    </div>
                                    ...
                                    <div className="grid md:grid-cols-2 gap-6">
                                        {section.posts.map((post) => (
                                            <article 
                                                key={post.id} 
                                                className={`rounded-2xl border flex flex-col group overflow-hidden transition-all duration-300 hover:border-slate-800/80 ${
                                                    isDarkMode 
                                                        ? 'bg-slate-900/30 border-slate-900 hover:bg-slate-900/60' 
                                                        : 'bg-white border-slate-200 shadow-sm'
                                                }`}
                                            >
                                                <Link href={`/blog/${post.slug}`} className="block overflow-hidden relative h-48 bg-slate-950/20">
                                                    <img
                                                ...
                                                <div className="p-6 flex flex-col flex-grow justify-between space-y-4">
                                                    <div className="space-y-2">
                                                        <div className="flex items-center gap-1 text-[10px] font-semibold text-slate-500 uppercase tracking-wider">
                                                            ...
                                                            <span>{post.published_at ?? 'Draft'}</span>
                                                        </div>
                                                        <h3 className="text-lg font-bold leading-snug group-hover:text-blue-500 transition-colors line-clamp-2">
                                                    ...
                                                    <Link 
                                                        href={`/blog/${post.slug}`} 
                                                        className="inline-flex items-center gap-1.5 text-xs font-bold text-blue-500 hover:text-blue-400 pt-1 group/btn"
                                                    >
                                                        Baca Selengkapnya
                        ...
                        <Pagination links={posts.links} isDarkMode={isDarkMode} />
                    </section>
                    ...
                    <aside className="lg:col-span-4 lg:sticky lg:top-24 h-max space-y-6">
                        <div className={`rounded-2xl border p-6 ${
                            ...
                            <span className="text-[10px] font-bold text-blue-500 uppercase tracking-wider block mb-2">Arsip Publikasi</span>
                            ...
                            {archives.length ? (
                                <ul className="space-y-2.5">
                                    ...
                                    {archives.map((archive) => {
                                        ...
                                        return (
                                            <li key={archive.year}>
                                                <Link
                                                    href={`/blog?year=${archive.param}#${getYearAnchor(archive.year)}`}
                                                    className={`flex items-center justify-between px-4 py-2 rounded-xl border transition-colors text-xs font-bold ${
                                                        isActive
                                                            ? 'border-blue-500/50 bg-blue-500/10 text-blue-400'
                                                            : isDarkMode
                                ...
                                <p className="text-xs text-slate-500">Belum ada arsip tahun yang tersedia.</p>
                            )}
                        </div>
                    </aside>
                </div>
